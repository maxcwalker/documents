...
class KatzerPlot(plotFunctions):
    ...
    def compute_wall_heat_flux(self,T,mu):
        ...
        Pr = 0.71
        Re = 950
        # q_dot = (-mu_wall/(0.4*M**2*Pr*Re))*dTdy
        q_dot = k*dTdy*(288/0.0027075)
        return q_dot
...
ax1.plot(x[0,:]*0.02075/400.0, q_dot)
ax1.set_ylabel('Wall heat flux [$W/m^2$]')
ax1.set_xlabel('position along x')
# ax1.set_title('Normalised wall heat flux for an impinging shoch on a flat plate')
ax1.set_title('Wall heat flux over a flat plate')
ax1.grid()
