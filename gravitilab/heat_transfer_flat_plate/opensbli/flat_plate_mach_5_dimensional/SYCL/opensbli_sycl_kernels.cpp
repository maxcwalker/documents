...
extern double Minf;
cl::sycl::buffer<double,1> *R_p=nullptr;
extern double R;
...
extern int niter;
cl::sycl::buffer<double,1> *pref_p=nullptr;
extern double pref;
...
void ops_decl_const_char(OPS_instance *instance, int dim, char const * type, int size, char * dat, char const * name ) {
  ...
  if (!strcmp(name,"R")) {
    if (R_p == nullptr) R_p = new cl::sycl::buffer<double,1>(cl::sycl::range<1>(dim));
    auto accessor = (*R_p).get_access<cl::sycl::access::mode::write>();
    for ( int d=0; d<dim; d++ ){
      accessor[d] = ((double*)dat)[d];
    }
  }
  ...
  if (!strcmp(name,"pref")) {
    if (pref_p == nullptr) pref_p = new cl::sycl::buffer<double,1>(cl::sycl::range<1>(dim));
    auto accessor = (*pref_p).get_access<cl::sycl::access::mode::write>();
    for ( int d=0; d<dim; d++ ){
      accessor[d] = ((double*)dat)[d];
    }
  }
